...
export class PurchasesPage {
  ...
  readonly totalExpenses = computed(() => this.data.purchases().reduce((s, p) => s + p.totalPrice, 0));
  ...
  openAdd() {
    ...
    ref.afterClosed().subscribe((p: Purchase | undefined) => { if (p) this.data.addPurchase(p); });
  }
}
