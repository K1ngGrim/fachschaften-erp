...
import { Chart, CategoryScale, LinearScale, BarElement, PointElement, LineElement, Filler, Tooltip, Legend } from 'chart.js';
...
export class FinancesPage {
  ...
  readonly inventoryValue = computed(() => this.data.products().reduce((s, p) => s + p.stock * p.purchasePrice, 0));
  readonly currentMonth = computed(() => this.monthly()[this.monthly().length - 1]);
  readonly profitMargin = computed(() => this.totalRevenue() > 0 ? ((this.profit() / this.totalRevenue()) * 100).toFixed(1) : '0');
  ...
  readonly barChartData = computed<ChartData<'bar'>>(() => ({
    ...
    datasets: [
      { label: 'Revenue', data: this.monthly().map(m => m.revenue), backgroundColor: '#e07b2a', borderRadius: 4 },
      { label: 'Expenses', data: this.monthly().map(m => m.expenses), backgroundColor: '#1a1d2e', borderRadius: 4 },
    ],
  }));
  ...
  readonly chartOptions: ChartOptions = {
    responsive: true,
    plugins: { legend: { position: 'top' }, tooltip: { callbacks: { label: ctx => this.formatCurrency(ctx.parsed.y ?? 0) } } },
